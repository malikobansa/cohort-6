...
const deployERC20Util = async () => {
  ...
  const CrowdToken = await CrowdTokenContract.deploy(); // deploy CrowdToken contract
  return { CrowdToken, owner, addr1, addr2, addr3 }; // return an instance of the deployed CrowdToken
};
...
describe("CrowdToken Token Test Suite", () => {
  ...
  describe("Minting", () => {
    it("Should allow the owner to mint token", async () => {
      const { CrowdToken, owner, addr1 } = await loadFixture(deployERC20Util);
      const amount = ethers.parseEther("1.0");
      await CrowdToken.connect(owner).mint(addr1.address, amount);
      expect(await CrowdToken.totalSupply()).to.equal(amount);
      expect(await CrowdToken.balanceOf(addr1.address)).to.equal(amount); // Use addr1.address
    });

    it("Should revert if a non-owner tries to mint token", async () => {
      const { CrowdToken, addr1 } = await loadFixture(deployERC20Util);
      const amount = ethers.parseEther("1.0");
      await expect(
    ...
    it("Should emit Mint events when minting", async () => {
      ...
      await expect(CrowdToken.connect(owner).mint(addr1.address, amount))
        ...
        .withArgs(addr1.address, amount); // Use addr1.address
    });
  });
});